...
<%
	...
	try{
		...
		String member = memGroup.get("member");
		
		//得到所有的成员
		ArrayList<HashMap<String, String>> students = memMana.getAllMemsByMember(member);
		...
		for(int i = 0;i < students.size();i++){
			...
			memberOption += "<option value="+ssid+">"+sname+"</option>";
		}
		
		//得到所有的模块
		temp = null;
		ArrayList<HashMap<String, String>> parts = memMana.getAllPartsByMemgid(mem_gid);
		System.out.println("parts:"+parts);
		for(int i = 0;i < parts.size();i++){
			temp = parts.get(i);
			String partid = temp.get("partid");
			String part_name = temp.get("part_name");
			partOption += "<option value="+partid+">"+part_name+"</option>";
		}
...
<html>
  ...
  <body>
		...
		<div id="upload">
			...
			<div id="newRsClass">
				<fieldset>
					...
					<form action="newPart" method="post">
						...
						<p><label>项目模块名</label><input type="text" name="part_name" /></p>
			...
			<div id="addFun">
				<fieldset>
					<legend>添加项目模块功能点</legend>
					<form action="newFun" method="post">
						<% 
							if(session.getAttribute("message") != null){
								out.print("<p style='color:red'>"+session.getAttribute("message")+"</p>");
								session.setAttribute("message", null);
						...
						%>
						<p><label>项目模块</label><select name="partid"><%=partOption %></select></p>
						<p><label>功能点</label><input type="text" name="fun_name" /></p>
						<p><label>开发时间(天)</label><input name="day" type="text"/></p>
						<p><label>成员姓名</label><select name="sid"><%=memberOption %></select></p>
						<p><input class='but' type="submit" name="sub" value="添加" /></p>
					</form>
				</fieldset>
			</div>
