...
public class AddRsToStageServelt extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try{
			...
			if(action.equals("addTo")){
				...
				staid = Integer.parseInt(req.getParameter("staid"));
				sta_name = req.getParameter("sta_name");
				...
				//将该资源添加到该课程中
				int result = this.spaceTea.addRs2Stage(rsid, staid);
				if(result > 0){
					req.getSession().setAttribute("message", "添加资源成功");
					req.getSession().setAttribute("url", "admin/staResManage.jsp?staid="+staid+"&sta_name="+sta_name+"&itemid="+itemid+"&item_name="+item_name+"&classid="+cclassid+"&class_name="+class_name);
					resp.sendRedirect("success.jsp");
				} else {
					req.getSession().setAttribute("message", "添加资源失败");
					resp.sendRedirect("admin/staResManage.jsp?staid="+staid+"&sta_name="+sta_name+"&itemid="+itemid+"&item_name="+item_name+"&classid="+cclassid+"&class_name="+class_name);
				}
			} else { //ajax异步请求
				int classid = 0;
	...
	public void upload(HttpServletRequest req, HttpServletResponse resp){
		...
		try {			
			...
			byte[] b = null;
			itemid = Integer.parseInt(su.getRequest().getParameter("itemid"));
			item_name = su.getRequest().getParameter("item_name");
			b = item_name.getBytes("ISO-8859-1");
			item_name = new String(b, "utf-8");
			...
			//9、获得当前的课程cid
			staid = Integer.parseInt(su.getRequest().getParameter("staid"));
			
			//10、获得class_name
			class_name = su.getRequest().getParameter("class_name");
			
			//10、获得classid,项目的分类id
			cclassid = Integer.parseInt(su.getRequest().getParameter("cclassid"));
			
			//11、得到sta_name
			sta_name = su.getRequest().getParameter("sta_name");
			...
			//把资源添加到resource中
			 ...
			 if(insertid > 0){
				 //将该资源添加到该课程中
				 int result = this.spaceTea.addRs2Stage(insertid, staid);
				 if(result > 0){
					 
					 req.getSession().setAttribute("message", "添加资源成功");
					 req.getSession().setAttribute("url", "admin/staResManage.jsp?staid="+staid+"&sta_name="+sta_name+"&itemid="+itemid+"&item_name="+item_name+"&cclassid="+classid+"&class_name="+class_name);
					 resp.sendRedirect("success.jsp");
					 return ;
				 }
			 } 
