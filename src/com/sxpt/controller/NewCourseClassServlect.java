...
public class NewCourseClassServlect extends HttpServlet {
	SpaceTeaModule spaceTM = null;
	public NewCourseClassServlect(){
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try{
			...
			int insertid = this.spaceTM.newCourseClass(class_name);
			
			if(insertid > 0){
				 req.getSession().setAttribute("message", "新建课程分类成功");
				 req.getSession().setAttribute("url", "admin/newCourseClass.jsp");
				 resp.sendRedirect("success.jsp");
			 } else {
				 req.getSession().setAttribute("message", "新建课程分类失败");
